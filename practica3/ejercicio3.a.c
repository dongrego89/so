...
void * consume(void * l);//Funcion consumidor
...
int main(){
	...
	error=pthread_create(&hilos[1],NULL,(void *)consume,limite);//Creacion de hilo consumidor
	validaCreate(error);
...
void * produce(void * l){
	...
	while(limit!=0){//Si no hemos llegado al limite de producciones..
